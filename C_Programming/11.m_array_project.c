...
int main(void)
{
	...
	initAnimalArray(); // 카드 배열 만들기
	initAnimalName();
	shuffleAnimal();  // 카드 지도에서 어떻게 배치할지
	...
	while (1)
	{
		...
		// 카드가 안 뒤집혔는지(checkAnimal = 0) and 두 동물이 같은지
		// 다른 동물인 경우
		if (
			// 안 뒤집혔는지 확인
...
void printQuestion() // 문제 출력 (카드 지도)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			...
			seq++;
			// 아직 뒤집지 못 했으면(정답을 못 맞혔으면) -> 위치를 나타내는 숫자
		}
		printf("\n");
	}
